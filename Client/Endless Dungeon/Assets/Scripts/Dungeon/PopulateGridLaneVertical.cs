...
public class PopulateGridLaneVertical : MonoBehaviour
{
            ...
            for (int y = 0; y < columns; y++)
            {
                if (IsALane(x, y))
                {
                    ...
                    if (!IsAlredyALane(tmpx, tmpy))
                        EditorManager.Instance.AddLaneVertical(x, y);
    ...
    {
        foreach (Lane lane in EditorManager.Instance.GetLaneVerticals())
        {
    ...
    bool IsALane(int x, int y)
    {
        foreach (Room room in EditorManager.Instance.GetRooms())
            if (room.GetCoordinates().x == x + 1 && room.GetCoordinates().y == y)
